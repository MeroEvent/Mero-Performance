...
export async function PUT(req: NextRequest) {
  try {
    ...
    const extendedPolicies: Record<string, any> = {
      ...
      annual_leave_quota: Number(body.annual_leave_quota ?? 1),
      home_leave_accrual_days: Number(body.home_leave_accrual_days ?? 1),
      mourning_leave_quota: Number(body.mourning_leave_quota ?? 13),
      comp_off_quota: Number(body.comp_off_quota ?? 5),
      lwp_quota: Number(body.lwp_quota ?? 0),
      maternity_leave_quota: Number(body.maternity_leave_quota ?? 60),
      max_carry_over_days: Number(body.max_carry_over_days ?? 90),
